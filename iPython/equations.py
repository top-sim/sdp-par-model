...
class Equations:
    ...
    def apply_imaging_equations(telescope_parameters, imaging_mode, bl_dep_time_av, On_the_fly=0, verbose=False):
        ...
        o.Theta_fov = 7.66 * o.wl * o.Qfov / (
            pi * o.Ds * o.Nfacet)  # Facet Field-of-view **PDR05 uses lambda_max, not mean
        ...
        o.epsilon_f_approx = sqrt(
            6. * (1. - (1.0 / o.amp_f_max)))  # expansion of sine to solve epsilon = arcsinc(1/amp_f_max).
        ...
        o.Nf_no_smear_predict = log(o.wl_max / o.wl_min) / log(
            (3. * o.wl / (2. * o.Bmax) / (o.Theta_fov * o.Nfacet * o.Qbw)) + 1.)  # Eq. 4 for full FOV, at max baseline
        o.Nf_no_smear_backward = log(o.wl_max / o.wl_min) / log((3. * o.wl / (2. * o.Bmax_bin) / (
            o.Theta_fov * o.Qbw)) + 1.)  # Eq. 4 for facet FOV. Ensures correct location of visibility on grid.
        ...
        if bl_dep_time_av:
            o.Tdump_predict = Min(o.Tdump_skipper, 1.2,
                                  o.Tion)  # use whatever is smaller. Don't let any BLdep averaging be for longer than 1.2s or Tion.
            o.Tdump_backward = Min(o.Tdump_skipper * o.Nfacet,
                                   o.Tion)  # For backward step at gridding only, allow coalescance of visibility points at Facet FoV smearing limit only for BLDep averaging case.
        else:
        ...
        o.DeltaW_SShot = o.Bmax_bin * o.Tsnap * o.Omega_E / (
            o.wl * 2.)  # Eq. 26 w-deviation for snapshot **PDR05 uses lambda_min, not mean
        ...
        o.Nvis_backward = o.binfrac * o.Na * (o.Na - 1) * o.Nf_vis_backward / (
            2. * o.Tdump_backward)  # Eq. 31 Visibility rate for backward step, allow coalescing in time and freq prior to gridding
        o.Nvis_predict = o.binfrac * o.Na * (o.Na - 1) * o.Nf_vis_predict / (
            2. * o.Tdump_predict)  # Eq. 31 Visibility rate for predict step
        ...
        o.Rfft_backward = o.binfrac * o.Nfacet ** 2 * 5. * o.Npix_linear ** 2 * log(o.Npix_linear,
                                                                                    2) / o.Tsnap  # Eq. 33, per output grid (i.e. frequency)
        o.Rfft_predict = o.binfrac * 5. * (o.Npix_linear * o.Nfacet) ** 2 * log((o.Npix_linear * o.Nfacet),
                                                                                2) / o.Tsnap  # Eq. 33 per predicted grid (i.e. frequency)
        o.Rfft_intermediate_cycles = ( o.Rfft_backward * o.Nf_FFT_backward) + ( o.Rfft_predict * o.Nf_FFT_predict )
        o.Rfft_final_cycle = ( o.Rfft_backward * o.Nf_out) + (
            o.Rfft_predict * o.Nf_FFT_predict )  # final major cycle, create final data products (at Nf_out channels)
        o.Rflop_fft = o.Nmajor * o.Nbeam * o.Npp * ((
                                                        o.Nmajor - 1) * o.Rfft_intermediate_cycles + o.Rfft_final_cycle)  # do nmajor-1 cycles before doing the final major cycle.
        ...
        o.Rflop_proj = (o.Nbeam * o.Npp) * ((
                                                o.Nmajor - 1) * o.Nf_FFT_backward + o.Nf_out)  # Reproj intermetiate major cycle FFTs (Nmaj -1) times, then do the final ones for the last cycle at the full output spectral resolution.

        # Make GCF
        o.grid_cell_error = o.epsilon_f_approx
        o.dfonF = o.grid_cell_error / o.Qkernel / (o.Ncvff / o.Qgcf)
        o.Nf_gcf_backward_nosmear = log(o.wl_max / o.wl_min) / log(
        ...
        if On_the_fly == 1:
            ...
            o.Nf_gcf_backward = Max(o.Nf_gcf_backward_nosmear,
                                    o.minimum_channels)  # maintain distributability, need at least minimum_channels (500) kernels.
            o.Nf_gcf_predict = Max(o.Nf_gcf_predict_nosmear,
                                   o.minimum_channels)  # maintain distributability, need at least minimum_channels (500) kernels.
        ...
        o.Rccf_backward = o.Nf_gcf_backward * o.Nfacet ** 2 * 5. * o.binfrac * (
            o.Na - 1) * o.Na * o.Nmm * o.Ncvff ** 2 * log(o.Ncvff, 2) / (2. * o.Tkernel_backward)  # Eq. 35
        o.Rccf_predict = o.Nf_gcf_predict * o.Nfacet ** 2 * 5. * o.binfrac * (
            o.Na - 1) * o.Na * o.Nmm * o.Ncvff ** 2 * log(o.Ncvff, 2) / (2. * o.Tkernel_predict)  # Eq. 35
        o.Rccf = o.Rccf_backward + o.Rccf_predict
        o.Rflop_conv = Rflop_common_factor * o.Rccf

        # Add in some phase rotation for the faceting:
        o.Rflop_phrot = o.Nmajor * o.Nbeam * o.Npp * (o.Nvis_predict + o.Nvis_backward) * o.Nfacet ** 2 * 25. * sign(
            o.Nfacet - 1)  # Eq. 29
        ...
        o.Rio = o.Mvis * (
            o.Nmajor + 1) * o.Nbeam * o.Npp * o.Nvis_predict * o.Nfacet ** 2  # added o.Nfacet dependence; changed Nmajor factor to Nmajor+1 as part of post PDR fixes.

...
\m  \left[ \mathrm{tdump}=0.3\;\mathrm{s} , N_{\rm a}=94 , N_{\rm f}=256000 , N_{\rm beam}=36 , D_{\rm s}=15\;\mathrm{m} , \lambda=.258\;{{\mathrm{m}}\over{\mathrm{s}\,\mathrm{Hz}}} , B_{\rm max}=50000.\;\mathrm{m} , \mathrm{NAA}=9 \right] \\
\end{maxima}
...
\m  \mathrm{GridRate(SKA1Survey)}=2.15 \times 10^{+12}\,N_{\rm facet}^2\,\left(N_{\rm GW}^2+81.0\right)\;{{\mathrm{Ops}}\over{\mathrm{s}}} \\
\end{maxima}
...
\m  \mathrm{CompRate(SKA1Survey)}=\left(2.15 \times 10^{+12}\,N_{\rm facet}^2\,\left(N_{\rm GW}^2+81.0\right)+{{5.73 \times 10^{+15}\,\log \left({{1.778 \times 10^{+8}}\over{N_{\rm facet}^2}}\right)}\over{N_{\rm facet}\,N_{\rm GW}}}+{{3.97 \times 10^{+16}}\over{N_{\rm facet}\,N_{\rm GW}}}\right)\;{{\mathrm{Ops}}\over{\mathrm{s}}} \\
\end{maxima}
...
ev(float(CompRate), cons(Nfacet=1, cons( NGW=44,  SKA1Mid) ));
ev(float(CompRate), cons(Nfacet=4, cons( NGW=11,  SKA1Mid) ));
ev(float(CompRate), cons( NGW=33,  SKA1Survey) );
\maximaoutput*
\m  \left(4.65 \times 10^{+14}\,N_{\rm facet}^2+{{1.43 \times 10^{+13}\,\log \left({{1.306 \times 10^{+8}}\over{N_{\rm facet}^2}}\right)}\over{N_{\rm facet}}}+{{9.92 \times 10^{+13}}\over{N_{\rm facet}}}\right)\;{{\mathrm{Ops}}\over{\mathrm{s}}} \\
\m  9.96 \times 10^{+15}\;{{\mathrm{Ops}}\over{\mathrm{s}}} \\
\m  1.13 \times 10^{+16}\;{{\mathrm{Ops}}\over{\mathrm{s}}} \\
\m  \left(2.51 \times 10^{+15}\,N_{\rm facet}^2+{{1.74 \times 10^{+14}\,\log \left({{1.778 \times 10^{+8}}\over{N_{\rm facet}^2}}\right)}\over{N_{\rm facet}}}+{{1.2 \times 10^{+15}}\over{N_{\rm facet}}}\right)\;{{\mathrm{Ops}}\over{\mathrm{s}}} \\
\end{maxima}
...
For comments on ``w-stacking'' see \cite{VoronkovCalim2010Gridding},
and \cite{2013A&A...553A.105T}. The concept of this technique is that
visibilities with different values of the $w$ coordinate are not
combined in the $uv$ domain but rather gridded separately and
transformed into the image (maybe we should be calling it the
``$lm$''?) domain, corrected for the $w$ term and then combined. This
obviously requires a degree of sorting of the data in the $w$
dimension.
...
\begin{enumerate}
  ...
  \item The whole observation is processed together. The largest $w$
    that needs to be controlled by (half) the separation of the $w$
    planes
  \item Instead of re-projection a correction of the $w$ term is
    required for each plane after the FFT to image plane
...
\begin{enumerate}
 \item The calculation does not currently include option of
   additionally faceting
\end{enumerate}
...
ev(float(wsGridRate), append([wsNWPlanes=128, NAA=9, Nfacet=1], SKA1Mid));
\maximaoutput*
...
\m  \mathrm{wsGridRate(SKA1Survey)}=2.15 \times 10^{+12}\,N_{\rm facet}^2\,\left({{2777778.}\over{N_{\rm facet}^2\,\mathrm{wsNWPlanes}^2}}+81.0\right)\;{{\mathrm{Ops}}\over{\mathrm{s}}} \\
\m  4.6 \times 10^{+15}\;{{\mathrm{Ops}}\over{\mathrm{s}}} \\
\end{maxima}
...
wstot: ComRate = (wsFFTTot+wsImCorrectTot)/tobs + wsGridRate;

float(solve(diff(ev(wstot, append([NAA=9, Nfacet=1, tobs=(6*3600`s)], SKA1Mid) ) / (1 `(Ops/s)), wsNWPlanes), wsNWPlanes));

float(ev(wstot, append([NAA=9, Nfacet=1, tobs=(6*3600`s), wsNWPlanes =
174], SKA1Mid)));


float(solve(diff(ev(wstot, append([NAA=9, Nfacet=1, tobs=(6*3600`s)], SKA1Low) ) / (1 `(Ops/s)), wsNWPlanes), wsNWPlanes));

\maximaoutput*
\m  \mathrm{ComRate}=\left({{{{320\,B_{\rm max}^2\,N_{\rm beam}\,N_{\rm f}\,\log \left({{16\,B_{\rm max}^2}\over{D_{\rm s}^2\,N_{\rm facet}^2}}\right)\,\mathrm{wsNWPlanes}}\over{\log 2\,D_{\rm s}^2}}+{{3200\,B_{\rm max}^2\,N_{\rm beam}\,N_{\rm f}\,\mathrm{wsNWPlanes}}\over{D_{\rm s}^2}}}\over{\mathrm{tobs}}}+{{8\,\left(N_{\rm a}-1\right)\,N_{\rm a}\,N_{\rm beam}\,N_{\rm f}\,N_{\rm facet}^2\,\left({{B_{\rm max}^2}\over{4\,D_{\rm s}^2\,N_{\rm facet}^2\,\mathrm{wsNWPlanes}^2}}+\mathrm{NAA}^2\right)}\over{\mathrm{tdump}}}\right)\;\mathrm{Ops} \\
\m  \left[ \mathrm{wsNWPlanes}=3.758 \times 10^{-6}\,\left(4.002 \times 10^{+7}\,i-2.311 \times 10^{+7}\right) , \mathrm{wsNWPlanes}=-3.758 \times 10^{-6}\,\left(4.002 \times 10^{+7}\,i+2.311 \times 10^{+7}\right) , \mathrm{wsNWPlanes}=174. \right] \\
\m  \mathrm{ComRate}=7.41 \times 10^{+15}\;{{\mathrm{Ops}}\over{\mathrm{s}}} \\
\m  \left[ \mathrm{wsNWPlanes}=7.396 \times 10^{-6}\,\left(2.736 \times 10^{+7}\,i-1.579 \times 10^{+7}\right) , \mathrm{wsNWPlanes}=-7.396 \times 10^{-6}\,\left(2.736 \times 10^{+7}\,i+1.579 \times 10^{+7}\right) , \mathrm{wsNWPlanes}=234. \right] \\
\end{maxima}
